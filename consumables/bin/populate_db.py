...
class GW2DB:
    ...
    def __init__(self):
        self.consumables_api = "http://www.gw2spidy.com/api/v0.9/json/all-items/3"
        self.detail_api = "https://api.guildwars2.com/v1/item_details.json?item_id="
        self.undead = "http://www.gw2spidy.com/api/v0.9/json/item/8878"

    def create(self):
        items = json.loads(urllib2.urlopen(self.consumables_api).read())['results']
        count = 0
        for item in items:
            try:
                id = item['data_id']
                official_dump = json.loads(urllib2.urlopen(self.detail_api + str(id)).read())
                ...
                pass
        print count
        #ds(urllib2.urlopen(self.undead).read())['result']
        #_id']
        # json.loads(urllib2.urlopen(self.detail_api + str(id)).r#
        #p['type'] == 'Consumable':
        #=id, name = official_dump['name'], slug = slugify(#l_dump['name']), consumable_type = 'UTIL', duration = #l_dump['consumable']['duration_ms'], description = #l_dump['consumable']['description'], buy_cost = item['#er_unit_price'], sell_cost = item['min_sale_unit_price']#
        #_dump['consumable']['type'] == 'food':
        #mable_type = 'FOOD'
        #
